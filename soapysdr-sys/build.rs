...
fn build_bundled_soapysdr() -> Vec<PathBuf> {
    ...
    if needs_clone {
        ...
        assert!(
            status.success(),
            "Failed to clone SoapySDR: git exited with status {}",
            status
        );

        std::fs::write(&revision_file, revision_to_build)
            .expect("Failed to write revision cache");
    }

    let lib_cmakelists = soapysdr_dir.join("lib/CMakeLists.txt");
    let lib_cmakelists_content = std::fs::read_to_string(&lib_cmakelists)
        .expect("Failed to read lib/CMakeLists.txt");
    ...
    let patched_content = lib_cmakelists_content.replace(
        "add_library(SoapySDR SHARED",
        "add_library(SoapySDR STATIC"
    );
    ...
    let config_h_content = std::fs::read_to_string(&config_h)
    ...
    std::fs::write(&config_h, patched_config)
        .expect("Failed to write patched Config.h");
    ...
    if cfg!(unix) {
        ...
        let cpp_lib = if cfg!(target_env = "musl") {
            ...
            "stdc++"
        } else if std::env::var("RUSTFLAGS").ok().map_or(false, |f| f.contains("libc++"))
            || std::env::var("RUSTFLAGS").ok().map_or(false, |f| f.contains("stdlib=libc++")) {
